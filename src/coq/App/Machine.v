...
(* Evaluate a SUB instruction with the given machine state. *)
Definition evalSUB (mach : Machine) : MResult :=
    match mach with
        | Mach [] r t => exception MachineErrorStackUnderflow
        | Mach (s :: ss) (Reg r) t =>
...
(* Evaluate a DIV instruction with the given machine state. *)
Definition evalDIV (mach : Machine) : MResult :=
    match mach with
        | Mach [] r t => exception MachineErrorStackUnderflow
        | Mach (s :: ss) (Reg r) t =>
